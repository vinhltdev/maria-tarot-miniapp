  ...
  return (
    <main className="min-h-screen bg-[#05020a] flex flex-col items-center justify-center p-4 text-white font-sans overflow-hidden relative">
      ...
      {mode === 'triple' && !isDrawing && drawnCards.length === 3 && drawnCards.every((c) => c.isFlipped) && tripleInterpretation && (
        <div className="mt-6 p-4 bg-[#1a0b2e]/80 border border-[#bb86fc]/20 rounded-xl max-w-md text-left z-10 animate-fade-in backdrop-blur-sm space-y-3">
          <h3 className="text-[#bb86fc] text-[10px] font-bold tracking-widest uppercase">Kiến giải trải bài thuần Việt</h3>

          <div>
            <h4 className="text-[10px] text-[#d9b8ff] font-bold uppercase tracking-wide">Điểm sáng cốt lõi</h4>
            <p className="text-[11px] text-gray-200 leading-relaxed mt-1">{tripleInterpretation.formatted.diemSangCotLoi}</p>
          </div>

          <div>
            <h4 className="text-[10px] text-[#ffb7b7] font-bold uppercase tracking-wide">Điều cần tránh</h4>
            <p className="text-[11px] text-gray-200 leading-relaxed mt-1">{tripleInterpretation.result.tongKet.dieuCanTranh}</p>
          </div>

          <div>
            <h4 className="text-[10px] text-[#b8ffd7] font-bold uppercase tracking-wide">Việc nên làm (24h/7 ngày/30 ngày)</h4>
            <ul className="mt-1 space-y-1">
              {tripleInterpretation.formatted.viecNenLam.map((action) => (
                <li key={action} className="text-[11px] text-gray-200 leading-relaxed">• {action}</li>
              ))}
            </ul>
          </div>
        </div>
